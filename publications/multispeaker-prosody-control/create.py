...
# Word augmentation
out.write('<a href="#title5" class="custom-a-href"><h2 style="text-align: left;" onclick="showSample(event)" id="title1">5) Single word augmentation</h2></a>\n')
mypath = f'audio/word'
out.write('<div class="sample-container" style="display:none">\n')
...
for fname in ['3196-PER', '3196-PER2']:
    if fname == '3196-PER2':
        fname = '3196-PER'
        ph_dict[fname] = '03'
        trans = transcript[fname].split()
        trans[3] = '<span style="color:red">acknowledged</span>'
    else:
        trans = transcript[fname].split()
        trans[1] = '<span style="color:red">Wentworth</span>'
    out.write('<div class="sample">\n')
    out.write('<div class="sample-title">\n')
    out.write('<span class="quotation">&ldquo;</span>\n')
    out.write('<div class="transcript">')
...
for fname in ['567-RWV', '978-SUM']:
    ...
    trans = transcript[fname].split()
    if fname == '567-RWV':
        trans[1] = '<span style="color:red">father</span>'
    else:
        trans[3] = '<span style="color:red">unaware</span>'
    out.write(' '.join(trans))
    out.write('</div>\n')
    out.write('</div>\n')
    # GT
...
out.write('<a href="#title6" class="custom-a-href"><h2 style="text-align: left;" onclick="showSample(event)" id="title1">6) Single phoneme augmentation (SAMPA representation)</h2></a>\n')
