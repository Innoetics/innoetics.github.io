...
for x in ['LJ011-0015', 'LJ014-0028', 'LJ012-0061', 'LJ008-0124']:
    ...
    out.write(f'<div class="r-number" onclick="togglePlay(document.getElementById(\'{x}_gt\'))">GT</div>\n')
    out.write(f'<audio id="{x}_gt" controls preload="none">\n')
    ...
    out.write(f'<div class="r-number" onclick="togglePlay(document.getElementById(\'{x}_gtvoc\'))">Voc</div>\n')
    out.write(f'<audio id="{x}_gtvoc" controls preload="none">\n')
    out.write(f'<source src="audio/gtvoc/{x}.wav" type="audio/wav">\n')
    out.write('Your browser does not support the audio element.\n')
    out.write('</audio>\n')
    out.write('</div>')
    out.write('</div>\n')
...
for i in range(1, len(temp) + 1):
    ...
    for r in ['2', '3', '5', '7', '10']:
        out.write('<div class="sample-audio">\n')
        out.write(f'<div class="r-number" onclick="togglePlay(document.getElementById(\'{i}_{r}\'))">r{r}</div>\n')
        out.write(f'<audio id="{i}_{r}" controls preload="none">\n')
